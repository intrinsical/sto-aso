...
public class ShipSelectionPanel extends JPanel {
	...
	public ShipSelectionPanel() {
		...
		GridBagConstraints gbc_pnlShips = new GridBagConstraints();
		gbc_pnlShips.weightx = 50.0;
		gbc_pnlShips.insets = new Insets(5, 5, 5, 5);
		gbc_pnlShips.fill = GridBagConstraints.BOTH;
		gbc_pnlShips.gridx = 0;
		gbc_pnlShips.gridy = 0;
		add(pnlShips, gbc_pnlShips);
		pnlShips.setLayout(new BorderLayout(0, 0));

		JXTaskPane taskPane = new JXTaskPane(LabelFilter);
		taskPane.setCollapsed(true);
		pnlShips.add(taskPane, BorderLayout.NORTH);
		...
		lstShips.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				Ship ship = lstShips.getSelectedValue();
				pnlDetails.setShip(ship);
			}
		});
		lstShips.setCellRenderer(shipCellRenderer);
		scrollPane.setViewportView(lstShips);

		pnlDetails = new ShipDetailsPanel();
		GridBagConstraints gbc_pnlDetails = new GridBagConstraints();
		gbc_pnlDetails.weightx = 50.0;
		gbc_pnlDetails.insets = new Insets(5, 5, 0, 0);
	...
	public static List<Ship> dialogAddOneTimeShips(Container container, PlayerFaction faction, String title) {
		...
		panel.addShips(new TreeSet<Ship>(Datastore.getAllShips().values()));
		panel.setTier6Only();
		switch (faction) {
		case Federation:
